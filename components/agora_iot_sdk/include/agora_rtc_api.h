...
*******************************************************************************/
  ...
  // Decryption error
  ERR_DECRYPTION = 31,
  // Custom packet length failed
  ERR_CUSTOM_PACKET_LENGTH_FAILED = 32,
...
*******************************************************************************/
  ...
  */
  ...
  /* create data stream not support reliable + unordered */
  ERR_DATA_STREAM_NOT_SUPPORTED = 124,
  /* too many data stream */
  ERR_TOO_MANY_DATA_STREAMS = 125,
  /* The data size is over 1024 */
  ERR_DATA_STREAM_SIZE_TOO_LARGE = 126,
  /* stream id not exist */
  ERR_NOT_EXIST_STREAM_ID = 127,
  /* The bitrate of the sent data exceeds the limit of 6 Kbps*/
  ERR_DATA_STREAM_BITRATE_LIMIT = 128,
  /* send data stream exceeds the limit of 60 PPS */
  ERR_DATA_STREAM_SEND_TOO_OFTEN = 129,
  /* peer data stream time out*/
  ERR_DATA_STREAM_TIME_OUT = 130,
...
/**
 * The definition of the packet_info_t struct.
 */
typedef struct {
  // The packet data. [in/out]
  void *data;
  // The packet data size. [in/out]
  uint16_t size;
} packet_info_t;

/**
 * The definition of the packet_type_e enum.
 */
typedef enum {
  // 0: audio packet type.
  PACKET_AUDIO = 0,
...
typedef struct {
  ...
  /**
   * Occur when receive incoming message for custom-defined
   * @param[in] packet_type     packet message type
   * @param[in/out] pkt_info    packet information data and size, both input and output
   */
void (*on_packet_incoming)(packet_type_e packet_type, packet_info_t *pkt_info);

  /**
   * Occur when sending outgoing message for custom-defined
   * @param[in] packet_type     packet message type
   * @param[in/out] pkt_info    packet information data and size, both input and output
   */
void (*on_packet_outgoing)(packet_type_e packet_type, packet_info_t *pkt_info);
...
*
* @param[in] conn_id Connection identification
* @param[out] streamId The pointer to the ID of the data stream.
...
*
* @param[in] streamId The ID of the sent data stream, returned in the "agora_rtc_create_data_stream" method.
